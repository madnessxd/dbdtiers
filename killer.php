...
<html>
  <head>
    ...
    <title>Dead by Daylight - Tier list - Killer Perks</title>
...
function ratePerk(perkId, ratingInp) {
    ...
    xhttp.open("POST", "https://dennisreep.nl/dbd/rateKiller.php?rating=" + ratingInp + "&perkId=" + perkId, true);
...
$stmt = $db->prepare('SELECT * FROM `KillerPerk` WHERE 1 ORDER BY `Rating` DESC');
$stmt->execute();
...
while ($row = $stmt->fetch(PDO::FETCH_NUM, PDO::FETCH_ORI_NEXT)) {
	...
	$charRow = '<td><a href ="' . $charUrlString . '"><img src="https://dennisreep.nl/dbd/KillerPortraits/56px-' . trim($row[4]) . '.png" alt="' . trim($row[4]) . '" /><h1>' . trim($row[4]) . '</a></h1></td>';
	...
	echo utf8_encode (
	$trStart . 
	'<td>' . '<img src="https://dennisreep.nl/dbd/KillerPerks/64px-IconPerks_' . $row[1] . '" alt="'. $row[1] .'"/></td>
